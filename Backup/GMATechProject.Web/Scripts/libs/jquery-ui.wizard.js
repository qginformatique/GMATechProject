    ...
    $.widget("db.jWizard", {
        ...
        */
        _create: function () {
            ...
            this.element.find(".ui-state-default").on("mouseover mouseout", function (event) {
        ...
        */
        _updateTitle: function (firstStep) {
            var wizard = this,
				...
				$currentStep = this.element.find(".jw-step:eq(" + this._stepIndex + ") legend");

            if (!firstStep) {
                this._effect($title, "title", "hide", "hide", function () {
                    $title.text($currentStep.text());
        ...
        _buildMenu: function () {
            ...
            this.element.find(".jw-step").each(function (x) {
                list.push($("<li />", {
                    ...
                    html: $("<a />", {
                        step: x,
                        text: $(this).find("legend").attr("title")
                    })
                })[0]);
            });
        ...
        */
        _updateCounter: function (firstStep) {
            ...
            if (counterOptions.type === "percentage") {
                ...
                counterText += " " + counterOptions.separatorText + " " + actualCount;
            } else {
                counterText = "N/A";
            }
        ...
        */
        _buildButtons: function () {
            var self = this,
				...
				$cancel = $('<button type="' + options.cancelType + '" class="btn btn-warning jw-button-cancel jw-priority-secondary' + (options.cancelHide ? " ui-helper-hidden" : "") + '">' + options.cancelText + '</button>'),
				$previous = $('<button type="button" class="btn jw-button-previous">' + options.previousText + '</button>'),
				$next = $('<button type="button" class="btn btn-primary jw-button-next">' + options.nextText + '</button>'),
				$finish = $('<button type="' + options.finishType + '" class="btn btn-success jw-button-finish">' + options.finishText + '</button>');
